...
class UserFlashcardProgressRepo {
  ...
  Future<DateTime?> getNextReviewDate(String uid) async {
    try {
      final doc = await _userFlashcardProgressService.getNextReviewDate(uid);

      return doc?.data()['nextReview'].toDate();
    } catch (e) {
      throw Exception("Could not fetch next review date from database");
    }
  }
}
